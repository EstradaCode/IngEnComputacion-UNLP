...
procedure actualizar(var maestro:ArchivoMaestro; var detalles: Detalles; var elems:rDetalles;var texto:text);
var min,act:rDetalle; rMae:Empleado; total_licencia:integer;
begin
minimo(detalles,elems,min);
read(maestro,rMae);
while(min.cod_emp <> VALOR_ALTO) do begin
	total_licencia:=0;
	act:= min;
	while (not EOF(maestro)) and (rMae.cod_emp <> act.cod_emp) do begin
		read(maestro,rMae);
	end;
	if( not eof(maestro)) then begin
		...
		if(rMae.dias_vacaciones < total_licencia) then
			writeln(texto, rMae.cod_emp, ' ', rMae.nombre, ' ', rMae.apellido, ' ', rMae.dias_vacaciones, ' ', total_licencia)
		else begin
...
for i:=1 to CANT_DETALLES do begin
	assign(dets[i], 'ej1detalles' + IntToStr(i) + '.dat');
	reset(dets[i]);
	leer(dets[i],elems[i]);
end;
