...
public class ProgramaCompra{
    public static void main (String[]args){
        ...
        System.out.println("Desea agregar una nueva Compra (SI/NO)?");
        sigo= in.next();
        if(! sigo.toUpperCase().equals("SI")){
            Sistem.out.println("--Fin del programa--");
            System.exit(0);
        }
        while ((diml < compras.length) && (sigo.toUpperCase().equals("SI"))){
            compras[diml]= LeerCompra(in);
            diml++;
            System.out.println();
    ...
    public static Vehiculo LeerVehiculo(Scanner in ){
        Compra compra= null;    
         int num; int categoria; String nombreEstadio; Fecha fecha, double importe;// de superclase;
         int dia,mes,anio;String nombre; int DNI,numAsiento;// de  clase v.i ;
        int opcion;// selector.
        num=
