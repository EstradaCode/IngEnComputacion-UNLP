...
function cantprod(l:listaNew):integer;
var cont:integer;
begin
     cont:=0;
     while(l<>nil) do begin
          if((l^.dato.anio >=2018) and ( l^.dato.anio <= 2020)) and (l^.dato.precio > 100000)then
              cont:=cont+1;
     l:=l^.sig;
    end;
end;

Procedure imprimirAmbercom(ll:listaL);
begin
     while(ll<> nil) do begin
          if( ll^.dato.marca == 'Ambercom') then
              writeln('Marca:', ll^.dato.marca);
              writeln('cantidad de productos:' , cantprod(ll^.dato.l));
     ll:=ll^.sig;
    end;
end;
