...
procedure ImprimirApellido(arb:arbol;a:cadena);
begin
  if(arb <> nil)then begin
    ImprimirApellido(arb^.hi,a); //1
    if( arb^.dato.apellido = a) then begin
        writeln(arb^.dato.nombre);
        writeln( arb^.dato.dni)
    end;
    ImprimirApellido(arb^.hd,a); //3
  end;
end;
...
begin
  ...
  readln(ape);
  writeln( 'los nombres y dnis de: ', ape, '  son: ');
  ImprimirApellido(arbolCurso,ape);
  readln;
end.
