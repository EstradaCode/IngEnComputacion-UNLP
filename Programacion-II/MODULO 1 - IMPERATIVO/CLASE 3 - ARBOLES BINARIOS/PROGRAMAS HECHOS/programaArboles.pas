...
var nue:arbol;
begin
  if( a = nil)then begin       {crea la raiz del arbol y despues sus hijos (recursion)}
    new(nue);
    nue^.dato:=dato;
    nue^.HI:= nil;
    nue^.HD:= nil;
...
procedure listaArbol(var a: arbol; l:lista);    {convierte los datos de una lista en un abo}
begin
 while (l<>nil) do begin
...
Procedure preOrden( a: arbol ); {recorrido de la estructura}
begin
  if ( a <> nil ) then begin
    write (a^.dato, '   ');
    preOrden (a^.HI);
...
Procedure enOrden( a: arbol ); {recorrido ordenado de la estructura (menor a mayor)}
begin
  if ( a <> nil ) then begin
    enOrden (a^.HI);
    write (a^.dato, '   ');
...
procedure postOrden(a:arbol); { recorrido desde los nodos más lejanos de los subarboles hasta la raiz}
begin
 if(a<>nil) then begin
   postOrden(a^.HI);
   postOrden(a^.HD);
